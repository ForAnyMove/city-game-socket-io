...
class Player{
	constructor(game, options){
		...
		if (options===undefined){
			const people = ['CapDude', 'BacketDude', 'HappyDude', 'KingDude'];
		...
		loader.load( `${game.assetsPath}fbx/people/${model}.fbx`, function ( object ) {
			...
			textureLoader.load(`${game.assetsPath}images/dude-texture-${colour}.jpg`, function(texture){
				object.traverse( function ( child ) {
					if ( child.isMesh ){
...
class PlayerLocal extends Player{
	constructor(game, model){
		...
		socket.on('deletePlayer', function(data){
			...
			if (players.length>0){
				index = game.remotePlayers.indexOf(players[0]);
				if (index!=-1){
